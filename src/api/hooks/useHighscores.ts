...
function toSummary(content: Highscore[], page: number, results: number): Highscores {
...
export default function useHighscores(page: number, results: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const content = await apiFetch<Highscore[]>(`/api/highscores?page=${page}&results=${results}`);
